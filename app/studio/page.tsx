...
export default function StudioPage() {
  ...
  const selectArea = (area: StudioArea) => {
    setActiveArea(area);
    setNotice(area === "games" ? "대문 · 게임 갤러리" : area === "teams" ? "Team · 팀 정보 나열" : `${area === "history" ? "History" : "Activities"} · 3개 콘텐츠`);
  };
  ...
  const addTeam = () => setTeams((current) => [...current, { id: makeId(), teamName: "새 팀", gameName: "게임 이름", description: "팀과 게임에 대한 설명을 입력해 주세요.", preview: "" }]);
  const updateTeam = (id: string, field: "teamName" | "gameName" | "description", value: string) => setTeams((current) => current.map((team) => team.id === id ? { ...team, [field]: value } : team));
  const replaceTeamImage = (id: string, event: ChangeEvent<HTMLInputElement>) => {
    ...
    setNotice("팀 사진을 선택했습니다. 변경사항 저장을 눌러주세요.");
    event.target.value = "";
  };
  ...
  const applyTeams = async () => {
    ...
    setTeams(savedTeams);
    setNotice(`${savedTeams.length}개 팀 정보를 저장했습니다. Team 페이지를 새로고침하면 반영됩니다.`);
  };
