...
export default function TeamPage() {
  ...
  return <main className="team-page">
    ...
    {teams.length ? <div className="team-list">{teams.map((team, index) => <section className={`team-entry ${index % 2 === 1 ? "is-media-left" : ""}`} id={`team-${team.id}`} key={team.id}><div className="team-entry-copy"><p className="section-index">{String(index + 1).padStart(2, "0")} / PROJECT</p><h2>{renderText(team.gameName || "프로젝트 이름을 입력해 주세요")}</h2><p>{renderText(team.description || "Studio에서 프로젝트 설명을 입력해 주세요.")}</p></div><div className="team-entry-media">{team.image ? <img src={withBasePath(`/${team.image}`)} alt={`${team.gameName} 대표 이미지`} /> : <div className="team-image-placeholder">PROJECT IMAGE</div>}</div></section>)}</div> : <section className="team-empty"><p>아직 등록된 프로젝트가 없습니다.</p><span>로컬 Studio의 ‘프로젝트 정보 나열’에서 첫 번째 프로젝트를 추가해 주세요.</span></section>}
